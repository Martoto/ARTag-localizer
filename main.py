...
if __name__ == '__main__':
    ...
    video_output = cv2.VideoWriter(str(output_destination), video_format, 20.0, (1280, 720))
    total_frames = 0
    ...
    while True:
        ...
        for contour in contours:
            ...
            if 2000 < contour_area < 10000 and len(contour_poly_curve) == 4:
                 ...
                 if ratio>=0.9 and ratio<=1.1 :
                    ...
                    _, result = si.get_h_matrices(contour_poly_curve, cols, rows, orientation)
                    grid = cv2.warpPerspective(detector.draw_grid([9,10]), result , (cols, rows))
                    ...
                    vf_original = cv2.merge(vf_original, grid)
                    ...
                    detector.draw_grid([9,10])
                    cv2.imwrite("teste.png", vf_original)
